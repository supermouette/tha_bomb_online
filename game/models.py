...
class Clicker(models.Model):
    ...
    def next_reward(self):
        r = (
            ClickerReward.objects.filter(threshold__gt=self.total)
            .order_by("threshold")
            .first()
        )
...
class NameToGuess(models.Model):
    ...
    def __str__(self):
        return f"{self.guess_date} - {self.name}"


class NameProposition(models.Model):
    name = models.CharField(max_length=30, null=False)
    popularity = models.IntegerField(default=1, null=False)
...
class ArrivalBet(models.Model):
    ...
    def __str__(self):
        return f"{self.name} ({self.date_bet})"
